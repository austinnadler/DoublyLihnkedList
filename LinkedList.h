...
public:
    ...
    E getFront() {
        if ( isEmpty() ) { throw std::logic_error("Error in getFront(): List is empty"); }
        return head->next->element;
    }

    // Return the last element in the list
    E getBack() {
        if ( isEmpty() ) { throw std::logic_error("Error in getBack(): List is empty"); }
    ...
    // Return true if the element given is found in the list once.
